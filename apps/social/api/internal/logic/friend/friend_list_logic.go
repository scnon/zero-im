...
func (l *FriendListLogic) FriendList(req *types.FriendListReq) (resp *types.FriendListResp, err error) {
	...
	friends, err := l.svcCtx.Social.FriendList(l.ctx, &social_client.FriendListReq{
		UserId: uid,
	})
	if err != nil {
		return nil, err
	...
	// 根据uids查询用户信息
	users, err := l.svcCtx.User.FindUser(l.ctx, &user_client.FindUserReq{
		Ids: uids,
	})
	if err != nil {
		return &types.FriendListResp{}, nil
	}
	userRecords := make(map[string]*user_client.UserEntity, len(users.Users))
